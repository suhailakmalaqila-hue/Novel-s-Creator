...
> = ({
  ...
  onDataRestored,
    ...
    const handleExportJSON = async () => {
      ...
      try {
        const response =
          await apiRequest<BackupExportData>(
            "/backup/export"
          );

        if (
          !response ||
          response.format !== "novels-creator" ||
          response.version !== 1 ||
          !response.project
        ) {
          throw new Error(
            "Data backup tidak valid atau tidak tersedia."
          );
        }
        ...
        <div className="relative w-full max-w-2xl bg-[#1E1E2E] border border-[#2A2A3C] rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex px-6 pt-3 bg-[#181826] border-b border-[#2A2A3C] gap-2">
            <button
              ...
              className={`pb-3 px-3 text-xs font-semibold border-b-2 transition-all cursor-pointer ${activeTab === 'profile'
            ...
            <button
              ...
              className={`pb-3 px-3 text-xs font-semibold border-b-2 transition-all cursor-pointer ${activeTab === 'writing'
                ? 'border-[#D4AF37] text-[#D4AF37]'
                : 'border-transparent text-[#8A8A9E] hover:text-[#E0E0E0]'
                }`}
            >
            ...
            <button
              ...
              className={`pb-3 px-3 text-xs font-semibold border-b-2 transition-all cursor-pointer ${activeTab === 'backup'
                ? 'border-[#D4AF37] text-[#D4AF37]'
                : 'border-transparent text-[#8A8A9E] hover:text-[#E0E0E0]'
                }`}
            >
