...
export const BookModal: React.FC<BookModalProps> = ({
  ...
  /*
   * Sinkronkan state modal setiap kali:
   * - modal dibuka
   * - buku yang diedit berubah
   * - berpindah dari Edit -> Create
   *
   * Ini mencegah data buku sebelumnya tertinggal di modal.
   */
  useEffect(() => {
    if (!isOpen) {
      return;
    ...
    setTargetWordCount(initialBook?.targetWordCount || 50000);
    setStatus(initialBook?.status || 'ongoing');
    setSelectedGenres(initialBook?.genres || []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
     ...
     * PENTING:
     *
     * Untuk buku BARU, jangan membuat ID lokal seperti:
     * book_123456789
     *
     * ID harus dibuat oleh backend/database.
     *
     * Untuk buku yang sedang diedit, ID existing tetap dikirim
     * supaya App dapat memilih updateBook().
     */
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-editorial text-lg font-bold text-[#FAF7EE]">
                {initialBook
                  ? 'Edit Detail Buku Cerita'
                  : 'Buat Buku Cerita Baru'}
              </h3>
            ...
            <div className="w-full sm:w-40 flex flex-col items-center shrink-0">
              <div className="relative group w-36 h-48 rounded-xl border-2 border-dashed border-[#3A3A54] hover:border-[#D4AF37]/60 bg-[#161624] overflow-hidden flex flex-col items-center justify-center shadow-lg transition-all">
                ...
                >
                  ...
                  <span>
                    {coverUrl ? 'Ganti Sampul' : 'Pilih Gambar'}
                  </span>
                </button>
              </div>
