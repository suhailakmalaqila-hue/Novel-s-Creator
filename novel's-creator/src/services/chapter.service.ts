...
function mapChapter(chapter: any): Chapter {
  return {
    id: chapter.id,
    ...
    lastSavedAt: chapter.last_saved_at
      ? new Date(chapter.last_saved_at).getTime()
      : 0,
    createdAt: new Date(chapter.created_at).getTime(),
  };
}

function mapSnapshot(snapshot: any): ChapterSnapshot {
  return {
    id: snapshot.id,
    chapterId: snapshot.chapter_id,
    bookId: snapshot.book_id,
    chapterTitle: snapshot.chapter_title,
    content: snapshot.content,
    wordCount: Number(snapshot.word_count),
...
export async function getChapters(bookId: string): Promise<Chapter[]> {
  const response = await apiRequest<ChaptersResponse>(
...
export async function deleteChapter(
  ...
  await apiRequest(`/books/${bookId}/chapters/${chapterId}`, {
    method: "DELETE",
  });
}
...
export async function getSnapshots(
  ...
  const response = await apiRequest<SnapshotsResponse>(
