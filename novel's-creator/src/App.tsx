...
import {
  getQuickNotes as getQuickNotesRequest,
  createQuickNote as createQuickNoteRequest,
  updateQuickNote as updateQuickNoteRequest,
  deleteQuickNote as deleteQuickNoteRequest,
...
function MainAppContent() {
  ...
  const handleSaveQuickNote =
    useCallback(
      async (
        ...
        if (isPersistedNote) {
          await updateQuickNoteRequest(
            note.id,
            note
          );
        } else {
          await createQuickNoteRequest(
            note
          );
        }
  ...
  const handleDataRestored =
    useCallback(async () => {
       ...
       * =========================================================
       *
       * refreshBooks sekarang mengembalikan daftar Book
       ...
       * =========================================================
       *
       * Jangan menunggu effect App bekerja sendiri.
       * Setelah import, langsung ambil chapter untuk
       * seluruh Book hasil restore.
       */
      if (restoredBooks.length > 0) {
        await Promise.all(
          restoredBooks.map(
            (book) =>
              refreshChapters(book.id)
          )
        );
      }
      ...
      /*
       * =========================================================
       * 3. REFRESH CHARACTER + QUICK NOTE
       * =========================================================
       *
       * Keduanya mempunyai lifecycle terpisah
       * dari BookContext.
       */
      await Promise.all([
        refreshCharacters(),
      ...
      /*
       * Tidak memanggil refreshUser().
       *
       * Backup hanya berisi data project,
       * bukan credential/data akun.
       */
    }, [
      refreshBooks,
      refreshChapters,
